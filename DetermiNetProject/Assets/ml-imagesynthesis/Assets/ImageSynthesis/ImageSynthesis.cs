...
public class ImageSynthesis : MonoBehaviour {
	...
	private CapturePass[] capturePasses = new CapturePass[] {
		...
		new CapturePass() { name = "_segmentation", supportsAntialiasing = false },
	...
	private IEnumerator WaitForEndOfFrameAndSave(string filenameWithoutExtension, string filenameExtension, int width, int height, int specificPass = -1)
	{
		yield return new WaitForEndOfFrame();
		Save(filenameWithoutExtension, filenameExtension, width, height, specificPass);
	}

	private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height, int specificPass = -1)
	{
		if (specificPass == -1)
		{
			foreach (var pass in capturePasses)
